...
export default function AnnouncementModal({ announcement, onClose }: { announcement: AnnouncementRow | null; onClose: () => void }) {
  const isEdit = !!announcement;
  const serverAction = isEdit ? updateAnnouncement.bind(null, announcement.id) : createAnnouncement;
  ...
  const defaultExpiry = announcement?.expiresAt ? new Date(announcement.expiresAt).toISOString().slice(0, 10) : "";

  return (
    <>
      <div ref={overlayRef} onClick={close} style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.7)" }} />
      <div ref={modalRef} style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 201, width: "min(620px,95vw)", maxHeight: "90vh", overflowY: "auto", background: "#000", border: "1px solid rgba(255,255,255,0.1)" }}>
        <div style={{ padding: "1.25rem 1.5rem", borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, background: "#000" }}>
          <span style={{ ...MONO, fontSize: "10px", letterSpacing: "0.35em", textTransform: "uppercase", color: "rgba(255,255,255,0.35)" }}>{isEdit ? "Edit Announcement" : "New Announcement"}</span>
        ...
        <form action={formAction} style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.35rem" }}>
            ...
            <input name="title" defaultValue={announcement?.title} style={inputStyle} onFocus={(e) => (e.currentTarget.style.borderBottomColor = "rgba(255,255,255,0.6)")} onBlur={(e) => (e.currentTarget.style.borderBottomColor = "rgba(255,255,255,0.25)")} />
            {state?.errors?.title && <span style={{ ...MONO, fontSize: "10px", color: "rgba(255,80,80,0.8)" }}>{state.errors.title[0]}</span>}
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.35rem" }}>
            <label style={{ ...MONO, fontSize: "9px", letterSpacing: "0.3em", textTransform: "uppercase", color: "rgba(255,255,255,0.3)" }}>Content (Markdown) *</label>
            <textarea name="content" defaultValue={announcement?.content} rows={8} style={{ ...inputStyle, resize: "vertical", lineHeight: 1.6 }} onFocus={(e) => (e.currentTarget.style.borderBottomColor = "rgba(255,255,255,0.6)")} onBlur={(e) => (e.currentTarget.style.borderBottomColor = "rgba(255,255,255,0.25)")} />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", alignItems: "center" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.35rem" }}>
              <label style={{ ...MONO, fontSize: "9px", letterSpacing: "0.3em", textTransform: "uppercase", color: "rgba(255,255,255,0.3)" }}>Expires On</label>
              <input name="expiresAt" type="date" defaultValue={defaultExpiry} style={inputStyle} onFocus={(e) => (e.currentTarget.style.borderBottomColor = "rgba(255,255,255,0.6)")} onBlur={(e) => (e.currentTarget.style.borderBottomColor = "rgba(255,255,255,0.25)")} />
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", paddingTop: "1.2rem" }}>
              <input type="checkbox" name="isPinned" id="isPinned" defaultChecked={announcement?.isPinned} style={{ accentColor: "#fff", width: "14px", height: "14px" }} />
              <label htmlFor="isPinned" style={{ ...MONO, fontSize: "10px", letterSpacing: "0.2em", textTransform: "uppercase", color: "rgba(255,255,255,0.4)", cursor: "pointer" }}>Pin to top</label>
            </div>
          </div>
